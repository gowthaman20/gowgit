...
class BillWindow:
    def __init__(self, customer_name, customer_contact, items, master):
        ...
        self.bill_text.insert('end', "-"*115+"\n")
        self.bill_text.insert('end', "No   Product Name\t\t\t\t\t\t   Quantity\t\t     Rate(\u20B9)\t\t\tgst(\u20B9)\t\tTotal(\u20B9)\n")
        self.bill_text.insert('end', "-"*115+"\n")
        # gst Price set to 0 before calculating
        gst = 0.0
        ...
        for i in range(len(items)):
            item = items[i]
            self.bill_text.insert('end',f"\n  {i+1}  {item[0]}\t\t\t\t\t\t\t{item[1]}\t\t{item[2]}\t\t{item[3]}\t\t{item[4]}")
            total_col = item[4]
