...
class SalesWindow(tk.Frame, ThemeEngine):
    def __init__(self, master):
        ...
        self.unit_price.grid(row=6, column=1, pady=10, sticky="NSEW")
         # Price gst
        price_label = ttk.Label(selection_frame, text="gst Price",
                                 font="Arial 16 bold")
        price_label.grid(row=7, column=0, padx=(20,0), pady=10, sticky="W")

        self.gst_price = ttk.Label(selection_frame, font="Arial 16 bold")
        self.gst_price.grid(row=7, column=1, pady=10, sticky="NSEW")


        # Total Price
        tprice_label = ttk.Label(selection_frame, text="Total Price",
                                 font="Arial 16 bold")
        tprice_label.grid(row=8, column=0, padx=(20,0), pady=10, sticky="W")

        self.total_price = ttk.Label(selection_frame, font="Arial 16 bold")
        self.total_price.grid(row=8, column=1, pady=7, sticky="NSEW")
        ...
        self.add_to_cart_btn.place(x=310,y=400)
        ...
        self.cart.grid(row=1,column=0, padx=(10,0), ipady=13)

        self.cart.heading('one', text='Product Name')
        self.cart.column('one', minwidth=420, width=420, anchor="w", stretch=False)
        ...
        self.cart.heading('four', text='GST')
        self.cart.column('four', minwidth=90, width=90, anchor="c", stretch=False)


        self.cart.heading('five', text='Total Price')
        self.cart.column('five', minwidth=90, width=90, anchor="c", stretch=False)
    ...
    def get_quantity_entered(self, *args):
        ...
        if self.qty_entry.get().replace(" ","") == '':
            ...
            try:
                ...
                #print (Gst)
                gst_price = 0
                gst_price = float(unit_price*0.18*quantity_entered)
                # print(unit_price)
                total_price = 0
                total_price = quantity_entered*unit_price+gst_price
                self.gst_price.config(text=f"\u20B9 {gst_price}")
    ...
    def add_to_cart(self):
        ...
        gst_price = self.gst_price.cget("text")
        ...
        if quantity_entered < 1:
            ...
            self.cart.insert("", 'end', text ="", values =(product_selected, quantity_entered, unit_price, gst_price, total_price))
